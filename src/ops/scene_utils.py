...
def count_current_usage():
    ...
    texture_count = len([img for img in bpy.data.images if not _is_internal_image(img)])

    max_height = 0.0
    for obj in bpy.data.objects:
        if obj.type == "MESH" and not obj.hide_viewport and obj.data and len(obj.users_collection) > 0:
