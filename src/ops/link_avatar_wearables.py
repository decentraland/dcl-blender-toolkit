...
class OBJECT_OT_link_avatar_wearables(bpy.types.Operator):
    ...
    bl_description = (
        "Import and create editable Decentraland avatar collections for wearable development (Shape A and B)"
    )
    ...
    def execute(self, context):
        ...
        try:
            ...
            if imported_collections:
                ...
                self.report(
                    {"INFO"}, f"Successfully created editable avatar collections: {', '.join(editable_collections)}"
                )
