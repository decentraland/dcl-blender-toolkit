...
class OBJECT_OT_rename_textures(bpy.types.Operator):
    ...
    bl_description = (
        "Automatically rename textures based on their usage in materials (Base Color, HRM, Normal, Emissive)"
    )
    ...
    def analyze_material_nodes(self, material):
        ...
        for node in nodes:
            if node.type == "TEX_IMAGE" and node.image:
                ...
                if usage:
                    texture_usage[texture_name] = {"usage": usage, "material_name": material.name}
            ...
            for link in output.links:
                ...
                if target_node.type == "BSDF_PRINCIPLED":
                    ...
                    if link.to_socket.name == "Base Color":
                        return "baseColor"
                    elif link.to_socket.name == "Roughness" or link.to_socket.name == "Metallic":
                        return "hrm"
                    elif link.to_socket.name == "Normal":
                        return "normal"
                    elif link.to_socket.name == "Emission" or link.to_socket.name == "Emission Color":
                        return "emissive"
